import date_utils from '../src/dateUtils';
...
test('Format: converts date object to string', () => {
    ...
    expect(date_utils.toString(date)).toBe('2017-09-18');
});

test('Format: converts date object to string', () => {
    const date = new Date('2016-02-29 16:08:34.3');
    expect(date_utils.toString(date, true)).toBe('2016-02-29 16:08:34.300');
});
...
test('StartOf', () => {
    ...
    const start_of_millisecond = date_utils.startOf(date, 'millisecond');
    expect(date_utils.toString(start_of_millisecond, true)).toBe(
        '2017-08-12 15:07:34.012'
    );

    const start_of_second = date_utils.startOf(date, 'second');
    expect(date_utils.toString(start_of_second, true)).toBe(
        '2017-08-12 15:07:34.000'
    );

    const start_of_minute = date_utils.startOf(date, 'minute');
    expect(date_utils.toString(start_of_minute, true)).toBe(
        '2017-08-12 15:07:00.000'
    );

    const start_of_hour = date_utils.startOf(date, 'hour');
    expect(date_utils.toString(start_of_hour, true)).toBe(
        '2017-08-12 15:00:00.000'
    );

    const start_of_day = date_utils.startOf(date, 'day');
    expect(date_utils.toString(start_of_day, true)).toBe(
        '2017-08-12 00:00:00.000'
    );

    const start_of_month = date_utils.startOf(date, 'month');
    expect(date_utils.toString(start_of_month, true)).toBe(
        '2017-08-01 00:00:00.000'
    );

    const start_of_year = date_utils.startOf(date, 'year');
    expect(date_utils.toString(start_of_year, true)).toBe(
